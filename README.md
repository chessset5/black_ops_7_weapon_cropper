...
`format.sh` is a shell script I used to keep the script in compliance with Ruff formatting/linting.

It was originally generated by google gemini but later adjusted.

Run it with `sh format.sh` if you have git bash setup properly in windows or `bash format.sh` on any posix system.
